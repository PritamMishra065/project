...
# Load datasets
sym_des = pd.read_csv(r".\datasets\symptoms_df.csv")
...
svc = pickle.load(open(r".\datasets\svc.pkl", 'rb'))
def helper(dis):
    ...
    wrkout = [wrkout for wrkout in wrkout.values if pd.notna(wrkout)]
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            my_precautions = [i for i in precautions]
